...
def collector(device_ip, device_port, ssh_user, ssh_pass):
    ...
    with ConnectHandler(**params) as ch:
        ...
        print(f"\n[!] Connection established with {device_ip}")
        
        # set terminal length to avoid issues with longer outputs
        ...
        # set output .txt filename based on device hostname
...
def main():
    ...
    parser.add_argument('-c', '--csv', required=True, help='CSV filename')    
